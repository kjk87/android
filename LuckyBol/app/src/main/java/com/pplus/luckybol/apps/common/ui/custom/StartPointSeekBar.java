...
public class StartPointSeekBar extends View {
    ...
    public StartPointSeekBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (thumbImageDrawable == null) {
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                thumbImageDrawable =  context.getDrawable(R.drawable.ic_write_edit_pointer);
            } else {
                thumbImageDrawable = getResources().getDrawable(R.drawable.ic_write_edit_pointer);
            }
        }
        ...
        if (thumbImagePressedDrawable == null) {
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                thumbImagePressedDrawable =  context.getDrawable(R.drawable.ic_write_edit_pointer);
            } else {
                thumbImagePressedDrawable = getResources().getDrawable(R.drawable.ic_write_edit_pointer);
            }
        }
        ...
        topHeight = getContext().getResources().getDimensionPixelSize(R.dimen.height_50);
    ...
    protected synchronized void onMeasure(int widthMeasureSpec,
        ...
        int height = thumbImage.getHeight()+ topHeight;
            ...
            case MotionEvent.ACTION_MOVE:
                if (isThumbPressed) {
                    ...
                    if (isDragging) {
                        ...
                        // Scroll to plus the motion event
                        pointerIndex = event.findPointerIndex(mActivePointerId);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final RectF rect = new RectF(padding,
                0.5f * (topHeight + getHeight() - lineHeight), getWidth() - padding,
                0.5f * (topHeight + getHeight() + lineHeight));
    ...
    private void drawThumb(float screenCoord, boolean pressed, Canvas canvas) {
        ...
        paint.setTextSize(getContext().getResources().getDimensionPixelSize(R.dimen.textSize_35pt));
        paint.setTextAlign(Paint.Align.CENTER);

        canvas.drawText(String.valueOf((int)normalizedToValue(normalizedThumbValue)), screenCoord, topHeight,  paint);
        canvas.drawBitmap(pressed ? thumbPressedImage : thumbImage, screenCoord
                        - thumbHalfWidth,
                (float) ((0.5f * (topHeight + getHeight())) - thumbHalfHeight), paint);
    }
}
