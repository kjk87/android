...
import com.pplus.luckybol.apps.goods.ui.AlertGoodsLikeCompleteActivity
import com.pplus.luckybol.apps.product.data.ProductDetailStickyHeaderAdapter
...
import com.pplus.luckybol.databinding.ActivityProductShipDetailBinding
import com.pplus.luckybol.databinding.ItemOptionBinding
import com.pplus.luckybol.databinding.ItemSelectedOptionBinding
...
//            val intent = Intent(this@ProductShipDetailActivity, PurchaseProductShipActivity::class.java)
    ...
    private fun setData() {
        ...
        ApiBuilder.create().getProductPrice(params).setCallback(object : PplusCallback<NewResultResponse<ProductPrice>> {
            override fun onResponse(call: Call<NewResultResponse<ProductPrice>>?,
                ...
                if (response?.data != null) {
                    ...
                    when(mProductPrice!!.productDelivery!!.method){
                        1->{
                            binding.textProductShipDetailDeliveryMethod.text = getString(R.string.word_delivery)
                        }
                        2->{
                    ...
                    if (mProductPrice!!.product!!.useOption != null && mProductPrice!!.product!!.useOption!!) {
                        ...
                        binding.layoutProductShipDetailNotOption.visibility = View.GONE
                        //                        layout_product_ship_detail_total_price.setBackgroundColor(ResourceUtil.getColor(this@ProductShipDetailActivity, R.color.color_f0f0f0))
                        getOption()
                    } else {
                        binding.scrollProductShipDetailOption.visibility = View.GONE
                        binding.viewProductShipDetailOptionBar.visibility = View.GONE
                        binding.layoutProductShipDetailNotOption.visibility = View.VISIBLE
                        //                        layout_product_ship_detail_total_price.setBackgroundColor(ResourceUtil.getColor(this@ProductShipDetailActivity, R.color.white))
                        setTotalPrice()
                    }
            ...
            override fun onFailure(call: Call<NewResultResponse<ProductPrice>>?,
                                   t: Throwable?,
                                   response: NewResultResponse<ProductPrice>?) {
                hideProgress()
            }
    ...
    private fun listCall(page: Int) {
        ...
        ApiBuilder.create().getProductReviewByProductPriceSeqNo(params).setCallback(object : PplusCallback<NewResultResponse<SubResultResponse<ProductReview>>> {
            ...
            override fun onResponse(call: Call<NewResultResponse<SubResultResponse<ProductReview>>>?,
                                    response: NewResultResponse<SubResultResponse<ProductReview>>?) {
                hideProgress()
            ...
            override fun onFailure(call: Call<NewResultResponse<SubResultResponse<ProductReview>>>?,
                                   t: Throwable?,
                                   response: NewResultResponse<SubResultResponse<ProductReview>>?) {
                hideProgress()
                mLockListView = false
    ...
    private fun getOption() {
        ...
        ApiBuilder.create().getProductOption(params).setCallback(object : PplusCallback<NewResultResponse<ProductOptionTotal>> {
            override fun onResponse(call: Call<NewResultResponse<ProductOptionTotal>>?,
                                    response: NewResultResponse<ProductOptionTotal>?) {
                hideProgress()
                if (response?.data != null) {
            ...
            override fun onFailure(call: Call<NewResultResponse<ProductOptionTotal>>?,
                                   t: Throwable?,
                                   response: NewResultResponse<ProductOptionTotal>?) {
                hideProgress()
            }
