...
import com.pplus.luckybol.apps.event.data.EventWinHeaderAdapter
...
//            type = arguments.getSerializable(Const.TYPE) as EnumData.AdsType
    ...
    override fun init() {
        ...
        mAdapter!!.mEventWinFragment = this
...
//        showProgress("")
    ...
    private fun listCall(page: Int) {
        ...
        mLockListView = true
        ApiBuilder.create().getWinListOnlyPresent(params).setCallback(object : PplusCallback<NewResultResponse<EventWin>> {
    ...
    val eventReplyLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        if(result.data != null){
            ...
            val eventWin = data.getParcelableExtra<EventWin>(Const.EVENT_WIN)
