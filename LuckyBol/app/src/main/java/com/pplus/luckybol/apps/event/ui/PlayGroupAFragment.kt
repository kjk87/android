...
class PlayGroupAFragment : BaseFragment<BaseActivity>() {
    ...
    private var mSearch: EventType.WinAnnounceType? = null //    private var mGroupSeqNo = 1

    override fun onCreate(savedInstanceState: Bundle?) {

        super.onCreate(savedInstanceState) //        mGroupSeqNo = arguments?.getInt(Const.GROUP)!!
    }
    ...
    override fun init() {
        ...
        binding.recyclerPlayA.layoutManager = mLayoutManager
        mAdapter = PlayAdapter()
        mAdapter!!.launcher = defaultLauncher
        binding.recyclerPlayA.adapter = mAdapter

        binding.recyclerPlayA.addOnScrollListener(object : RecyclerView.OnScrollListener() {
        ...
        binding.swipeRefreshPlayA.setOnRefreshListener(object : SwipeRefreshLayout.OnRefreshListener {
            override fun onRefresh() {
                getCount()
                binding.swipeRefreshPlayA.isRefreshing = false
            }
        })
    ...
    private fun getCount() {
        ...
        ApiBuilder.create().getEventCount(params).setCallback(object : PplusCallback<NewResultResponse<Int>> {
            ...
            override fun onResponse(call: Call<NewResultResponse<Int>>,
                ...
                if (mTotalCount == 0) {
                    binding.layoutPlayANotExist.visibility = View.VISIBLE
                } else {
                    binding.layoutPlayANotExist.visibility = View.GONE
                }
    ...
    companion object {
        ...
        fun newInstance(): PlayGroupAFragment {

            val fragment = PlayGroupAFragment() //            val args = Bundle() //            args.putSerializable(Const.GROUP, groupSeqNo) //            fragment.arguments = args
            return fragment
        }
    }
