...
import com.pplus.luckybol.databinding.ActivityProductReviewRegBinding
...
class ProductReviewRegActivity : BaseActivity(), ImplToolbar {
    ...
    private var mProductReview: ProductReview? = null

    //    private var mProductPrice: ProductPrice? = null
    private var mPurchaseProduct: PurchaseProduct? = null
    ...
    override fun initializeView(savedInstanceState: Bundle?) {
        ...
        mPurchaseProduct = intent.getParcelableExtra(Const.PURCHASE_PRODUCT)
        ...
        if (mPostMode == EnumData.MODE.UPDATE) {
            mProductReview = intent.getParcelableExtra(Const.DATA)
            if (mProductReview != null) {
                binding.editProductReviewRegContents.setText(mProductReview!!.review)

                if (mProductReview!!.imageList != null && mProductReview!!.imageList!!.isNotEmpty()) {

                    for (reviewImage in mProductReview!!.imageList!!) {
                        mImageAdapter!!.add(reviewImage.image!!)
                    }
                    mImageAdapter!!.notifyDataSetChanged()
                }

                binding.ratingProductReviewReg.rating = mProductReview!!.eval!!.toFloat()
            } else {
    ...
    inner class UploadCheckThread : Thread() {
        ...
        private fun showProgress(message: String) {
            ...
            runOnUiThread { this@ProductReviewRegActivity.showProgress(message) }
        }
    }
    ...
    private fun requestPost() {
        ...
        val params = ProductReview()
        if (mImageAdapter!!.getDataList() != null && mImageAdapter!!.getDataList()!!.size > 0) {
            val imageList = arrayListOf<ProductReviewImage>()
            var array = 0
            for (image in mImageAdapter!!.getDataList()!!) {
                array++
                val productReviewImage = ProductReviewImage()
        ...
        when (mPostMode) {
            ...
            EnumData.MODE.WRITE -> {
                ...
                params.pageSeqNo = mPurchaseProduct!!.pageSeqNo
                params.productSeqNo = mPurchaseProduct!!.productSeqNo
                params.productPriceSeqNo = mPurchaseProduct!!.productPriceData!!.seqNo
                params.purchaseProductSeqNo = mPurchaseProduct!!.seqNo


                showProgress("")
                ApiBuilder.create().insertProductReview(params).setCallback(object : PplusCallback<NewResultResponse<Any>> {
            ...
            EnumData.MODE.UPDATE -> {
                params.seqNo = mProductReview!!.seqNo
                params.memberSeqNo = mProductReview!!.memberSeqNo
                params.pageSeqNo = mProductReview!!.pageSeqNo
                params.productSeqNo = mProductReview!!.productSeqNo
                params.productPriceSeqNo = mProductReview!!.productPriceSeqNo
                params.purchaseProductSeqNo = mProductReview!!.purchaseProductSeqNo
                params.review = contents

                showProgress("")
                ApiBuilder.create().updateProductReview(params).setCallback(object : PplusCallback<NewResultResponse<Any>> {
                    override fun onResponse(call: Call<NewResultResponse<Any>>?,
                                            response: NewResultResponse<Any>?) {
        ...
        return OnToolbarListener { v, toolbarMenu, tag ->
            when (toolbarMenu) {
                ...
                ToolbarOption.ToolbarMenu.RIGHT -> if (tag == 1) {
                    ...
                    }).builder().show(this@ProductReviewRegActivity)
                }
            }
        }
    }
