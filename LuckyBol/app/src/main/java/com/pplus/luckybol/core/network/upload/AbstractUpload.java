...
//                        ((Attachment) resultResponse.getData()).setHeight(height);
    ...
    public void checkSelfNetwork() {
        ...
        if (!PplusCommonUtil.Companion.hasNetworkConnection()) {
            ...
            builder.addContents(new AlertData.MessageData(LuckyBolApplication.getContext().getString(R.string.msg_disconnected_network), AlertBuilder.MESSAGE_TYPE.TEXT, 1));
            builder.addContents(new AlertData.MessageData(LuckyBolApplication.getContext().getString(R.string.msg_check_network_status), AlertBuilder.MESSAGE_TYPE.TEXT, 2));
