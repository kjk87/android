...
import com.pplus.luckybol.databinding.ItemLottoEventBinding
...
class LottoEventAdapter : RecyclerView.Adapter<LottoEventAdapter.ViewHolder> {
    ...
    class ViewHolder(binding: ItemLottoEventBinding) : RecyclerView.ViewHolder(binding.root) {

        val image = binding.imageLottoEvent
        val text_time = binding.textLottoEventTime
        val layout_close = binding.layoutLottoEventClose
        val text_win_time = binding.textLottoEventWinTime
        val text_proceeding = binding.textLottoEventProceeding
        val image_win_result = binding.imageLottoEventWinResult
        val text_title = binding.textLottoEventTitle
        val text_win_condition = binding.textLottoEventWinCondition
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (currentMillis > winAnnounceDate) {
            Glide.with(holder.itemView.context).load(item.winImageUrl).apply(RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE).fitCenter().placeholder(R.drawable.luckybol_default_img).error(R.drawable.luckybol_default_img)).into(holder.image)
            holder.layout_close.visibility = View.GONE
            isClose = false
            isWinAnnounce = true
        } else {
            Glide.with(holder.itemView.context).load(item.bannerImageUrl).apply(RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE).fitCenter().placeholder(R.drawable.luckybol_default_img).error(R.drawable.luckybol_default_img)).into(holder.image)
        }
        ...
        if (isClose && !isWinAnnounce) {
            ...
            if (remainWinMillis > 0) {
                ...
                Glide.with(holder.itemView.context).load(item.winImageUrl).apply(RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE).centerCrop().placeholder(R.drawable.luckybol_default_img).error(R.drawable.luckybol_default_img)).into(holder.image)
                holder.layout_close.visibility = View.GONE
                isClose = false
                isWinAnnounce = true
            }
...
//                holder.text_proceeding.visibility = View.VISIBLE
        ...
        holder.itemView.setOnClickListener {
            if (isClose) {
                PplusCommonUtil.showEventAlert(mContext!!, 0, item, null)
            } else {
                if (listener != null) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemLottoEventBinding.inflate(LayoutInflater.from(parent.context), parent, false)
