...
import com.pplus.luckybol.databinding.ItemProductShoppingGroupBinding
 ...
 */
class ProductShoppingGroupAdapter() : RecyclerView.Adapter<ProductShoppingGroupAdapter.ViewHolder>() {
    ...
    class ViewHolder(binding: ItemProductShoppingGroupBinding) : RecyclerView.ViewHolder(binding.root) {

        val layout_mask = binding.layoutProductShoppingGroupMask
        val image = binding.imageProductShoppingGroupImage
        val text_product_name = binding.textProductShoppingGroupName
        val text_discount_ratio = binding.textProductShoppingGroupDiscountRatio
        val text_sale_price = binding.textProductShoppingGroupSalePrice
        val text_origin_price = binding.textProductShoppingGroupOriginPrice
        val text_save_point = binding.textProductShoppingGroupSavePoint
        val image_like = binding.imageProductShoppingGroupLike
        val text_free_ship = binding.textProductShoppingGroupFreeShip
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (item.product!!.imageList != null && item.product!!.imageList!!.isNotEmpty()) {
            Glide.with(holder.itemView.context).load(item.product!!.imageList!![0].image).apply(RequestOptions().centerCrop().placeholder(R.drawable.luckybol_default_img).error(R.drawable.luckybol_default_img)).into(holder.image)
        } else {
        ...
        if(item.isPoint != null && item.isPoint!!){
            holder.text_save_point.visibility = View.VISIBLE
            holder.text_save_point.text = holder.itemView.context.getString(R.string.format_save_cash, FormatUtil.getMoneyTypeFloat(item.point!!.toString()))
        }else{
...
//        holder.text_discount.visibility = View.GONE
        ...
        if(item.productDelivery?.type == EnumData.DeliveryType.free.type){
            holder.text_free_ship.visibility = View.VISIBLE
        }else{
            holder.text_free_ship.visibility = View.GONE
        }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ItemProductShoppingGroupBinding.inflate(LayoutInflater.from(parent.context), parent, false)
