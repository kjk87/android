...
import com.pplus.luckybol.core.network.model.dto.BusinessLicense
import com.pplus.luckybol.core.network.model.dto.ProductPrice
import com.pplus.luckybol.core.network.model.response.NewResultResponse
import com.pplus.luckybol.databinding.ActivityProductSellerInfoBinding
...
class ProductSellerInfoActivity : BaseActivity(), ImplToolbar {
    ...
    override fun initializeView(savedInstanceState: Bundle?) {
        ...
        if (productPrice.product!!.imageList != null && productPrice.product!!.imageList!!.isNotEmpty()) {
            Glide.with(this).load(productPrice.product!!.imageList!![0].image).apply(RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE).centerCrop().placeholder(R.drawable.luckybol_default_img).error(R.drawable.luckybol_default_img)).into(binding.imageProductSellerInfoProductImage)
        }
