...
class ProductShipAdapter() : RecyclerView.Adapter<ProductShipAdapter.ViewHolder>() {
    ...
    class ViewHolder(binding: ItemProductShipBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        val text_free_ship = binding.textProductShipFreeShip
...
//            }
//
//            holder.text_count.text = PplusCommonUtil.fromHtml(holder.itemView.context.getString(R.string.html_format_count_per2, soldCount.toString(), item.product!!.count.toString()))
...
//
        if(item.isPoint != null && item.isPoint!!){
            ...
            holder.text_save_point.text = PplusCommonUtil.fromHtml(holder.itemView.context.getString(R.string.html_save_cash, FormatUtil.getMoneyTypeFloat(item.point!!.toString())))
        }else{
