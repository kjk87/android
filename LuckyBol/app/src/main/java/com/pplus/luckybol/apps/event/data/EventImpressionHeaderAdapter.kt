...
import com.pplus.luckybol.apps.event.ui.EventWinDetailActivity
...
import com.pplus.luckybol.databinding.HeaderEventImpressionBinding
...
class EventImpressionHeaderAdapter : RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    class ViewHolder(binding: ItemEventImpressionBinding) : RecyclerView.ViewHolder(binding.root) {
        ...
        val text_name = binding.textEventImpressionName
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder is ViewHeader) {
            ...
            if (item.primaryType == EventType.PrimaryType.number.name) {
                ...
                holder.text_win_code.text = mContext?.getString(R.string.format_event_win_code, item.winCode)
            } else {
                holder.text_win_code.visibility = View.GONE
            }

            holder.text_title.text = item.title //            if (item.winAnnounceType.equals(EventType.WinAnnounceType.immediately.name)) {
            //                holder.text_count.text = mContext?.getString(R.string.format_real_time_event_join_count, item.joinCount)
            //            } else {
            //                holder.text_count.text = mContext?.getString(R.string.format_event_join_count, item.joinCount)
            //            }

            if (item.winAnnounceType.equals(EventType.WinAnnounceType.immediately.name)) {
                holder.text_count.text = mContext?.getString(R.string.format_real_time_event_winner_count, item.winnerCount)
            } else {
                holder.text_count.text = mContext?.getString(R.string.format_event_winner_count, item.winnerCount)
            }
                ...
                for (i in 0..(mGiftList!!.size - 1)) {
                    ...
                    if (gift.type == "visit" && mMyWinList != null) {
                        for (myWin in mMyWinList!!) {
                            if (gift.seqNo == myWin.gift!!.seqNo) {
...
//                holder.text_gift_name.text = item.gift!!.title
//            } else {
//                holder.text_gift_name.text = PplusCommonUtil.fromHtml(mContext!!.getString(R.string.html_bol_unit, FormatUtil.getMoneyTypeFloat(item.amount.toString())))
//            }


            if (item.gift!!.type == "bol" || item.gift!!.type == "point") {
