...
class EventBuyCompleteActivity : BaseActivity() {
    ...
    var mEventBuy: EventBuy? = null

    override fun initializeView(savedInstanceState: Bundle?) {
        mEvent = intent.getParcelableExtra(Const.EVENT)
        mEventBuy = intent.getParcelableExtra(Const.EVENT_BUY)
        ...
        binding.textPointBuyWithEventCompletePoint.text = getString(R.string.format_save_point, FormatUtil.getMoneyType(mEventBuy!!.totalPrice.toString()))
        ...
        ApiBuilder.create().getGiftAll(params).setCallback(object : PplusCallback<NewResultResponse<EventGift>> {
            override fun onResponse(call: Call<NewResultResponse<EventGift>>?,
                ...
                if (response?.datas != null && response.datas!!.isNotEmpty()) {
                    ...
                    Glide.with(this@EventBuyCompleteActivity).load(eventGift.giftImageUrl).apply(RequestOptions().fitCenter().placeholder(R.drawable.luckybol_default_img).error(R.drawable.luckybol_default_img)).into(binding.imagePointBuyWithEventComplete)
                }
            }
            ...
            override fun onResponse(call: Call<NewResultResponse<Event>>?,
                ...
                if (mEvent != null) {
                    ...
                    if (mEvent!!.primaryType == EventType.PrimaryType.randomluck.name) {
                        Glide.with(this@EventBuyCompleteActivity).load(mEvent!!.totalGiftImageUrl).apply(RequestOptions().fitCenter().placeholder(R.drawable.luckybol_default_img).error(R.drawable.luckybol_default_img)).into(binding.imagePointBuyWithEventComplete)
                    } else {
                        getGiftAll()
                    }
        ...
        ApiBuilder.create().getMyJoinCount(params).setCallback(object : PplusCallback<NewResultResponse<Int>> {
            override fun onResponse(call: Call<NewResultResponse<Int>>?,
                ...
                if (response?.data != null) {
                    ...
                    binding.textPointBuyWithEventCompleteJoinCountTotal.text = getString(R.string.format_point_buy_join_count, mEventBuy!!.count.toString(), myJoinCount.toString())
                }
