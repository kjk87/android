...
import com.pplus.luckybol.apps.signin.ui.ChangePWActivity
import com.pplus.luckybol.apps.signin.ui.FindPWActivity
...
import com.pplus.luckybol.databinding.ActivityAccountConfigBinding
...
class AccountConfigActivity : BaseActivity(), ImplToolbar {
    ...
    private fun setProfileData() {
        if (LoginInfoManager.getInstance().user != null) {
            ...
            // 인증된 번호
            binding.textAccountPhoneNumber.text = user.mobile?.replace(Const.APP_TYPE+"##", "")
    ...
    fun changePassword(verification: Verification?) {
        ...
        intent.putExtra(Const.LOGIN_ID, LoginInfoManager.getInstance().user.loginId?.replace(Const.APP_TYPE+"##", ""))
        intent.flags = Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_SINGLE_TOP
        startActivity(intent)
    }
    ...
    private fun changeMobile(verification: Verification, mobile: String) {
        ...
        ApiBuilder.create().updateMobileByVerification(params).setCallback(object : PplusCallback<NewResultResponse<Any>> {
            override fun onResponse(call: Call<NewResultResponse<Any>>?,
                ...
                hideProgress()
                LoginInfoManager.getInstance().user.mobile = Const.APP_TYPE+"##"+mobile
