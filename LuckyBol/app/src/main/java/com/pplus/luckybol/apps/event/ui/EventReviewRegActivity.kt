...
import com.pplus.luckybol.databinding.ActivityEventReviewRegBinding
...
class EventReviewRegActivity : BaseActivity(), ImplToolbar {
    override fun getPID(): String {
        return ""
    }

    private lateinit var binding: ActivityEventReviewRegBinding

    override fun getLayoutView(): View {
        binding = ActivityEventReviewRegBinding.inflate(layoutInflater)
        return binding.root
    }

    private var mPostMode: EnumData.MODE? = null
    private var mEventReview: EventReview? = null
    ...
    override fun initializeView(savedInstanceState: Bundle?) {
        ...
        binding.pagerEventReviewRegImage.adapter = mImageAdapter
        binding.pagerEventReviewRegImage.offscreenPageLimit = 10

        if (mPostMode == EnumData.MODE.UPDATE) {
            mEventReview = intent.getParcelableExtra(Const.DATA)
            if (mEventReview != null) {
                binding.editEventReviewRegContents.setText(mEventReview!!.review)

                if (mEventReview!!.imageList != null && mEventReview!!.imageList!!.isNotEmpty()) {

                    for(eventReviewImage in mEventReview!!.imageList!!){
                        mImageAdapter!!.add(eventReviewImage.image!!)
                    }
                    mImageAdapter!!.notifyDataSetChanged()
                }
            } else {
                binding.layoutEventReviewRegImage.visibility = View.GONE
            }
            checkImage()
        }

        binding.layoutEventReviewRegRegPhoto.setOnClickListener {
            goPostGallery()
        }

        binding.editEventReviewRegContents.setOnTouchListener(object : View.OnTouchListener {
            override fun onTouch(v: View?, event: MotionEvent?): Boolean {
                if (binding.editEventReviewRegContents.length() > 0) {
    ...
    fun checkImage() {
        if (mImageAdapter!!.count > 0) {
            binding.layoutEventReviewRegImage.visibility = View.VISIBLE
        } else {
            binding.layoutEventReviewRegImage.visibility = View.GONE
        }
    }
    ...
    fun setImageRefresh(pos: Int) {
        binding.pagerEventReviewRegImage.currentItem = 0
    }
    ...
    private fun isEmptyData(): Boolean {
        ...
        val contents = binding.editEventReviewRegContents.text.toString().trim()

        if (StringUtils.isEmpty(contents)) {
            ToastUtil.show(this, R.string.msg_input_event_review_contents)
            return true
        }

        if (contents.length < 10) {
            ToastUtil.show(this, R.string.msg_input_event_review_over_10)
            return true
        }
    ...
    inner class UploadCheckThread : Thread() {
        ...
        private fun showProgress(message: String) {
            ...
            runOnUiThread { this@EventReviewRegActivity.showProgress(message) }
        }
    }
    ...
    private fun requestPost() {
        ...
        val params = EventReview()
        if (mImageAdapter!!.getDataList() != null && mImageAdapter!!.getDataList()!!.size > 0) {
            val eventReviewImageList = arrayListOf<EventReviewImage>()
            var array = 0
            for(image in mImageAdapter!!.getDataList()!!){
                array++
                val eventReviewImage = EventReviewImage()
                eventReviewImage.array = array
                eventReviewImage.image = image
                eventReviewImageList.add(eventReviewImage)
            }
            params.imageList = eventReviewImageList
        }
        ...
        when (mPostMode) {
            ...
            EnumData.MODE.WRITE -> {
                ...
                params.eventGiftSeqNo = mEventWin!!.gift!!.seqNo


                showProgress("")
                ApiBuilder.create().insertEventReview(params).setCallback(object : PplusCallback<NewResultResponse<Any>> {
                    override fun onResponse(call: Call<NewResultResponse<Any>>?, response: NewResultResponse<Any>?) {

                        hideProgress()
                        showAlert(R.string.msg_registered_event_review_post)
                        setResult(RESULT_OK)
                        finish()
                    }
            ...
            EnumData.MODE.UPDATE -> {
                params.seqNo = mEventReview!!.seqNo
                params.memberSeqNo = mEventReview!!.memberSeqNo
                params.eventSeqNo = mEventReview!!.eventSeqNo
                params.eventWinSeqNo = mEventReview!!.eventWinSeqNo
                params.eventWinId = mEventWin!!.id
                params.eventGiftSeqNo = mEventReview!!.eventGiftSeqNo
                params.review = contents

                showProgress("")
                ApiBuilder.create().updateEventReview(params).setCallback(object : PplusCallback<NewResultResponse<Any>> {
                    override fun onResponse(call: Call<NewResultResponse<Any>>?, response: NewResultResponse<Any>?) {

                        hideProgress()
                        showAlert(R.string.msg_modified_event_review)
                        setResult(RESULT_OK)
                        finish()
                    }
    ...
    override fun getToolbarOption(): ToolbarOption {
        ...
        toolbarOption.initializeDefaultToolbar(getString(R.string.word_event_review_write), ToolbarOption.ToolbarMenu.LEFT)
        ...
        return OnToolbarListener { v, toolbarMenu, tag ->
            when (toolbarMenu) {
                ...
                ToolbarOption.ToolbarMenu.RIGHT -> if (tag == 1) {
                    ...
                    val builder = AlertBuilder.Builder()
                    builder.setTitle(getString(R.string.word_notice_alert))
                    builder.addContents(AlertData.MessageData(getString(R.string.msg_question_event_review_write), AlertBuilder.MESSAGE_TYPE.TEXT, 2))
                    builder.setLeftText(getString(R.string.word_cancel)).setRightText(getString(R.string.word_confirm))
                    builder.setOnAlertResultListener(object : OnAlertResultListener {
                    ...
                    }).builder().show(this@EventReviewRegActivity)
                }
            }
        }
    }
