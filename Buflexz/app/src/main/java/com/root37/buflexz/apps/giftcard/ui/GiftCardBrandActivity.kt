...
import com.root37.buflexz.databinding.ActivityGiftCardBrandBinding
import com.root37.buflexz.databinding.ItemTopRightBinding
...
class GiftCardBrandActivity : BaseActivity(), ImplToolbar {
    ...
    private fun loginCheck() {
        if (LoginInfoManager.getInstance().isMember()) {
            ...
            Glide.with(this).load(Uri.parse("file:///android_asset/flags/${LoginInfoManager.getInstance().member!!.nation!!.uppercase()}.png")).into(binding.imageGiftCardBrandFlag)


            val nation = NationManager.getInstance().nationMap!![LoginInfoManager.getInstance().member!!.nation]
            if(nation!!.code == "KR"){
                binding.textGiftCardBrandNation.text = nation.name
            }else{
                binding.textGiftCardBrandNation.text = nation.nameEn
            }
