...
import com.pplus.prnumberuser.databinding.ItemGalleryOneBinding
import java.util.*

/**
 * Created by 김종경 on 2015-06-17.
 */
class GalleryOneAdapter(private val context: Context) :
    RecyclerView.Adapter<GalleryOneAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener {
            if (!item.isBroken!!) {
                ...
                if (mBeforePosition != holder.adapterPosition) {
                    mBeforePosition = holder.adapterPosition
                    mGalleryList[holder.adapterPosition].checked = true
                    (context as OneGalleryActivity).setSelect(item)
                    notifyDataSetChanged()
                }
            } else {
                Toast.makeText(context, R.string.msg_do_not_attach_broken_image, Toast.LENGTH_SHORT)
                    .show()
            }
        }
        if (item.checked!!) {
