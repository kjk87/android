...
class AlertBuySubscriptionQrActivity : BaseActivity() {
    ...
    override fun initializeView(savedInstanceState: Bundle?) {
        ...
        binding.textAlertSaveQrDesc.setText(R.string.msg_alert_buy_subscription_qr_desc)

        val dataString = "prnumberbiz://qr?memberSeqNo=${LoginInfoManager.getInstance().user.no}&type=subscriptionDown&productPriceSeqNo=${productPrice!!.seqNo}"
        makeQr(dataString)
        ...
        ApiBuilder.create().makeQrCode(params).setCallback(object : PplusCallback<NewResultResponse<String>> {
            override fun onResponse(call: Call<NewResultResponse<String>>?, response: NewResultResponse<String>?) {
                ...
                if (response?.data != null) {
                    ...
                    Glide.with(this@AlertBuySubscriptionQrActivity).load(url).apply(RequestOptions().centerCrop().placeholder(R.drawable.img_qr_default).error(R.drawable.img_qr_default)).into(binding.imageAlertSaveQr)
                }
            }
