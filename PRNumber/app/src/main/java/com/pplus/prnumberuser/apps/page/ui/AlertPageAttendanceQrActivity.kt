...
import com.pplus.prnumberuser.core.network.model.dto.PageAttendance
import com.pplus.prnumberuser.core.network.model.response.NewResultResponse
import com.pplus.prnumberuser.databinding.ActivityAlertSaveQrBinding
import retrofit2.Call

class AlertPageAttendanceQrActivity : BaseActivity() {
    ...
    override fun initializeView(savedInstanceState: Bundle?) {
        ...
        val pageAttendance = intent.getParcelableExtra<PageAttendance>(Const.PAGE_ATTENDANCE)

        binding.textAlertSaveQrDesc.setText(R.string.msg_alert_save_qr_desc)

        val dataString = "prnumberbiz://qr?memberSeqNo=${LoginInfoManager.getInstance().user.no}&type=attendance&pageAttendanceSeqNo=${pageAttendance!!.seqNo}"
        makeQr(dataString)
        ...
        ApiBuilder.create().makeQrCode(params).setCallback(object : PplusCallback<NewResultResponse<String>> {
            override fun onResponse(call: Call<NewResultResponse<String>>?, response: NewResultResponse<String>?) {
                ...
                if (response?.data != null) {
                    ...
                    Glide.with(this@AlertPageAttendanceQrActivity).load(url).apply(RequestOptions().centerCrop().placeholder(R.drawable.img_qr_default).error(R.drawable.img_qr_default)).into(binding.imageAlertSaveQr)
                }
            }
