...
import com.pplus.prnumberuser.Const.DEBUG_MODE
...
import com.pplus.prnumberuser.apps.setting.ui.FAQActivity
import com.pplus.prnumberuser.apps.signin.ui.AddInfoActivity
...
import com.pplus.prnumberuser.databinding.ActivityLauncherScreenBinding
...
class LauncherScreenActivity : BaseActivity() {
                ...
                override fun onInstallReferrerSetupFinished(responseCode: Int) {
                    when (responseCode) {
                        ...
                        InstallReferrerClient.InstallReferrerResponse.FEATURE_NOT_SUPPORTED -> { // API not available on the current Play Store app
                            LogUtil.e(LOG_TAG, "FEATURE_NOT_SUPPORTED")
                        }
    ...
    private fun gpaCompAction(gad_tracking_id: String) {
        ...
        ApiController.getGpaApi().gpaComp(params).enqueue(object : Callback<Any> {
    ...
    private fun appStart() {
        ...
        if (data != null) {
            PRNumberApplication.setSchemaData(data.toString())
        }

        if (checkPlayServices()) { // UserInfo init
            LoginInfoManager.getInstance()
            ApiController.getPRNumberService().updateHeaders()

            FirebaseMessaging.getInstance().token.addOnCompleteListener(OnCompleteListener { task ->
                if (!task.isSuccessful) {
                    LogUtil.e(LOG_TAG, "Fetching FCM registration token failed", task.exception)
    ...
    private fun checkSelfPermission(): Boolean {
        ...
        val locationPermission = ContextCompat.checkSelfPermission(this@LauncherScreenActivity, Manifest.permission.ACCESS_FINE_LOCATION) //        int callPhonePermission = ContextCompat.checkSelfPermission(LauncherScreenActivity.this, Manifest.permission.CALL_PHONE);

        val checkPermission: Boolean //        if(Const.API_URL.startsWith("https://api")){
        ...
        //        }
        //        checkPermission = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && (contactPermission == PackageManager.PERMISSION_DENIED || locationPermission == PackageManager.PERMISSION_DENIED)
        checkPermission = Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && (locationPermission == PackageManager.PERMISSION_DENIED)
        if (checkPermission) {
    ...
    private fun checkAppVersion() {
        ...
        ApiBuilder.create().appVersion(params).setCallback(object : PplusCallback<NewResultResponse<AppVersion>> {
            ...
            override fun onResponse(call: Call<NewResultResponse<AppVersion>>, response: NewResultResponse<AppVersion>) {
                ...
                } else if (isUpdate == 1) { //                        stopAnimation();
    ...
    fun userLoginCheck() {
        ...
        ApiBuilder.create().categoryFirstList.setCallback(object : PplusCallback<NewResultResponse<CategoryFirst>> {
            ...
            override fun onResponse(call: Call<NewResultResponse<CategoryFirst>>?, response: NewResultResponse<CategoryFirst>?) {
            ...
            override fun onFailure(call: Call<NewResultResponse<CategoryFirst>>?, t: Throwable?, response: NewResultResponse<CategoryFirst>?) {
                if (LoginInfoManager.getInstance().isMember) {
                    if (AppInfoManager.getInstance().isAutoSingIn) {
                        login()
                    } else {
    ...
    private fun getCategoryMajorOnly() {
        ApiBuilder.create().categoryMajorOnlyList.setCallback(object : PplusCallback<NewResultResponse<CategoryMajor>> {
    ...
    private fun loginCheck(data: User) {
        ...
        LoginResultManager2.getInstance().success(data) { state ->
            when (state) {
                ...
                LoginResultManager2.loginState.Cancel_btn, LoginResultManager2.loginState.Exile -> logOutAndSignIn()
            }
        }
    }
