...
public class MobileGiftHistoryAdapter extends RecyclerView.Adapter<MobileGiftHistoryAdapter.ViewHolder>{
    ...
    public void onBindViewHolder(final ViewHolder holder, int position){
        ...
        Glide.with(mContext).load(item.getMobileGift().getBaseImage()).apply(new RequestOptions().centerCrop().placeholder(R.drawable.prnumber_default_img).error(R.drawable.prnumber_default_img)).into(holder.image);
        holder.text_name.setText(item.getMobileGift().getName());
        holder.text_price.setText(mContext.getString(R.string.format_product_price, FormatUtil.getMoneyType(String.valueOf(PplusCommonUtil.Companion.calculateBol(item.getMobileGift().getSalesPrice())))));
