...
import com.pplus.prnumberuser.core.sns.naver.NaverUtil
import com.pplus.prnumberuser.core.util.PplusCommonUtil
import com.pplus.prnumberuser.databinding.FragmentSnsLoginBinding
    ...
    var mGoogleSignListener = object : GoogleUtil.GoogleSignListener{
        override fun handleSignInResult(account: GoogleSignInAccount?) {
            ...
            if(account != null){
                ...
                params["loginId"] = id
                params["password"] = Crypt.encrypt(SnsTypeCode.google.name + "-" + id)
        ...
        binding.layoutSnsLoginNaver.setOnClickListener {
            NaverUtil.click(activity, object : NaverUtil.NaverCallbackListener {
                override fun onSuccess(token: String?, content: String?) {
                    ...
                    params["password"] = Crypt.encrypt(SnsTypeCode.naver.name + "-" + email)
                        ...
                        if (error != null) {
                            ...
                            UserApiClient.instance.me { user, error ->
                                if (error != null) {
                                    ...
                                    LogUtil.e(LOG_TAG, "이메일: ${user.kakaoAccount?.email}")

                                    if(StringUtils.isNotEmpty(user.kakaoAccount?.email)){
                                        val params = java.util.HashMap<String, String>()
                                        params["loginId"] = user.kakaoAccount?.email!!
                                        params["password"] = Crypt.encrypt(SnsTypeCode.kakao.name + "-" + user.kakaoAccount?.email)
