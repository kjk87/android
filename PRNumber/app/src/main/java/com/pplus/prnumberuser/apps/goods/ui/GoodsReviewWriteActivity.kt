...
import com.pplus.prnumberuser.apps.goods.data.GoodsReviewRegImagePagerAdapter
...
import com.pplus.prnumberuser.databinding.ActivityGoodsReviewWriteBinding
...
class GoodsReviewWriteActivity : BaseActivity(), ImplToolbar {
    override fun getPID(): String {
        return ""
    }

    private var mPostMode: EnumData.MODE? = null
    private var mGoodsReview: GoodsReview? = null
    private var mEventWin:EventWin? = null

    private var mImageAdapter: GoodsReviewRegImagePagerAdapter? = null
    ...
    private lateinit var binding: ActivityGoodsReviewWriteBinding

    override fun getLayoutView(): View {
        binding = ActivityGoodsReviewWriteBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    override fun initializeView(savedInstanceState: Bundle?) {
        ...
        mImageAdapter = GoodsReviewRegImagePagerAdapter(this)
        binding.pagerGoodsReviewWriteImage.adapter = mImageAdapter
        binding.pagerGoodsReviewWriteImage.offscreenPageLimit = 10

        if (mPostMode == EnumData.MODE.UPDATE) {
            mGoodsReview = intent.getParcelableExtra(Const.DATA)
            if (mGoodsReview != null) {
                binding.editGoodsReviewWriteContents.setText(mGoodsReview!!.review)

                if (mGoodsReview!!.attachments != null && mGoodsReview!!.attachments!!.images != null && mGoodsReview!!.attachments!!.images!!.isNotEmpty()) {
                    mImageAdapter!!.mDataList = mGoodsReview!!.attachments!!.images!! as ArrayList<String>
                    mImageAdapter!!.notifyDataSetChanged()
                }
            } else {
                binding.layoutGoodsReviewWriteImage.visibility = View.GONE
            }
            binding.ratingGoodsReviewWrite.rating = mGoodsReview!!.eval!!.toFloat()
            checkImage()
        }

        binding.ratingGoodsReviewWrite.setOnRatingBarChangeListener { ratingBar, rating, fromUser ->
            if (rating <= 1.0) {
        ...
        binding.layoutGoodsReviewWriteRegPhoto.setOnClickListener {
            goPostGallery()
        }

        binding.editGoodsReviewWriteContents.setOnTouchListener(object : View.OnTouchListener {
            override fun onTouch(v: View?, event: MotionEvent?): Boolean {
                if (binding.editGoodsReviewWriteContents.length() > 0) {
        ...
        mImageAdapter!!.setOnItemChangeListener(object : GoodsReviewRegImagePagerAdapter.OnItemChangeListener {
    ...
    fun checkImage() {
        if (mImageAdapter!!.count > 0) {
            binding.layoutGoodsReviewWriteImage.visibility = View.VISIBLE
        } else {
            binding.layoutGoodsReviewWriteImage.visibility = View.GONE
        }
    }
    ...
    fun setImageRefresh(pos: Int) {
        binding.pagerGoodsReviewWriteImage.currentItem = 0
    }
    ...
    private fun isEmptyData(): Boolean {
        ...
        val eval = binding.ratingGoodsReviewWrite.rating.toInt()
        ...
        val contents = binding.editGoodsReviewWriteContents.text.toString().trim()
    ...
    inner class UploadCheckThread : Thread() {
        ...
        private fun showProgress(message: String) {
            ...
            runOnUiThread { this@GoodsReviewWriteActivity.showProgress(message) }
        }
    }
    ...
    private fun requestPost() {
        ...
        when (mPostMode) {
            ...
            EnumData.MODE.WRITE -> {
                ...
                val params = GoodsReview()
                params.memberSeqNo = LoginInfoManager.getInstance().user.no
                params.review = contents
                params.eval = binding.ratingGoodsReviewWrite.rating.toInt()
                if(mEventWin != null){
                    params.pageSeqNo = mEventWin!!.event!!.pageSeqNo
                    params.eventSeqNo = mEventWin!!.event!!.no
                    params.eventWinSeqNo = mEventWin!!.winNo
                    params.eventGiftSeqNo = mEventWin!!.gift!!.giftNo
                }
                ...
                showProgress("")
                ApiBuilder.create().postGoodsReview(params).setCallback(object : PplusCallback<NewResultResponse<Any>> {
            ...
            EnumData.MODE.UPDATE -> {
                ...
                showProgress("")
                ApiBuilder.create().putGoodsReview(params).setCallback(object : PplusCallback<NewResultResponse<Any>> {
                    override fun onResponse(call: Call<NewResultResponse<Any>>?, response: NewResultResponse<Any>?) {
            ...
            override fun onClick(v: View?, toolbarMenu: ToolbarOption.ToolbarMenu?, tag: Any?) {
                when (toolbarMenu) {
                    ...
                    ToolbarOption.ToolbarMenu.RIGHT -> if (tag == 1) {
                        ...
                        }).builder().show(this@GoodsReviewWriteActivity)
                    }
                }
            }
        }
