...
public class CropImageView extends AppCompatImageView{
    ...
    private void drawEditFrame(Canvas canvas) {
        ...
        if (mShowGuide) {
            ...
            float h1 = mFrameRect.left + (mFrameRect.right - mFrameRect.left) / 1.0f;
            float h2 = mFrameRect.right - (mFrameRect.right - mFrameRect.left) / 1.0f;
            float v1 = mFrameRect.top + (mFrameRect.bottom - mFrameRect.top) / 1.0f;
            float v2 = mFrameRect.bottom - (mFrameRect.bottom - mFrameRect.top) / 1.0f;
...
//
//    public CropInfo getMyCroppedBitmap() {
//        Bitmap source = getBitmap();
//        if (source == null) return null;
//
//        int x, y, w, h;
...
//    }
    ...
    public Bitmap getMyBitmap(int x, int y, int w, int h) {
        Bitmap source = getBitmap();
        Bitmap cropped = Bitmap.createBitmap(source, x, y, w, h, null, false);
        return cropped;
    }
    ...
    public enum RotateDegrees {
        ROATATE_NORMAL(0), ROTATE_90D(90), ROTATE_180D(180), ROTATE_270D(270);
