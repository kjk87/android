...
class VerifyWhatsAppActivity : BaseActivity(), ImplToolbar {
    ...
    override fun initializeView(savedInstanceState: Bundle?) {
        ...
        binding.textVerifyWhatsAppMobileNumber.text = getString(R.string.format_verify_code_desc1, mMobileNumber)
    ...
    private fun verifyForWithdrawal(otp:String){
        ...
        ApiBuilder.create().withdrawal(params).setCallback(object : PplusCallback<NewResultResponse<Any>> {
            override fun onResponse(call: Call<NewResultResponse<Any>>?, response: NewResultResponse<Any>?) {
                hideProgress()
                setEvent("withdrawal")
                val intent = Intent(this@VerifyWhatsAppActivity, WithdrawalCompleteActivity::class.java)
            ...
            override fun onFailure(call: Call<NewResultResponse<Any>>?, t: Throwable?, response: NewResultResponse<Any>?) {
                ...
                if (response?.code == 506) {
                    binding.editVerifyWhatsAppMobileOtp.startAnimation(AnimationUtils.loadAnimation(this@VerifyWhatsAppActivity, R.anim.shake))
    ...
    private fun verifyForWithdrawalCancel(otp:String){
        ...
        ApiBuilder.create().withdrawalCancelWithVerify(params).setCallback(object : PplusCallback<NewResultResponse<Any>> {
            override fun onResponse(call: Call<NewResultResponse<Any>>?, response: NewResultResponse<Any>?) {
                ...
                val intent = Intent(this@VerifyWhatsAppActivity, WithdrawalCancelCompleteActivity::class.java)
                intent.flags = Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_SINGLE_TOP
                startActivity(intent)
            }
    ...
    private fun verify(member:Member, otp:String){
        member.verifyType = "whatsapp"
        ...
        params["otp"] = otp
        showProgress("")
        ApiBuilder.create().verifyWhatsapp(params).setCallback(object  : PplusCallback<NewResultResponse<Any>> {
            override fun onResponse(call: Call<NewResultResponse<Any>>?, response: NewResultResponse<Any>?) {
                hideProgress()
                val intent = Intent(this@VerifyWhatsAppActivity, JoinNicknameActivity::class.java)
            ...
            override fun onResponse(call: Call<NewResultResponse<Member>>?, response: NewResultResponse<Member>?) {
                ...
                if (response?.result != null) {
                    if(response.result!!.status == "active"){
                            ...
                            override fun onResult(event_alert: AlertBuilder.EVENT_ALERT) {
                                when (event_alert) {
                                    AlertBuilder.EVENT_ALERT.RIGHT -> {
                                        val intent = Intent(this@VerifyWhatsAppActivity, WithdrawalCancelAuthActivity::class.java)
                        ...
                        }).builder().show(this@VerifyWhatsAppActivity)
                    }
                }
            }
