...
import com.lejel.wowbox.databinding.ItemNotificationBoxBinding
...
class NotificationBoxAdapter() : RecyclerView.Adapter<NotificationBoxAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if(item.isRead != null && item.isRead!!){
            holder.binding.textNotificationBoxTitle.setTextColor(ResourceUtil.getColor(holder.itemView.context, R.color.color_cdcdcd))
            holder.binding.textNotificationBoxContents.setTextColor(ResourceUtil.getColor(holder.itemView.context, R.color.color_cdcdcd))
            holder.binding.textNotificationBoxDate.setTextColor(ResourceUtil.getColor(holder.itemView.context, R.color.color_cdcdcd))
            holder.binding.imageNotificationBox.setImageResource(R.drawable.ic_alarm_read)
        }else{
            holder.binding.textNotificationBoxTitle.setTextColor(ResourceUtil.getColor(holder.itemView.context, R.color.color_333333))
            holder.binding.textNotificationBoxContents.setTextColor(ResourceUtil.getColor(holder.itemView.context, R.color.color_454545))
            holder.binding.textNotificationBoxDate.setTextColor(ResourceUtil.getColor(holder.itemView.context, R.color.color_454545))
