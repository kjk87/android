...
import com.lejel.wowbox.databinding.ActivityEventReplyEditBinding
...
class EventReplyEditActivity : BaseActivity(), ImplToolbar {
    ...
    override fun initializeView(savedInstanceState: Bundle?) {
        mEventReply = PplusCommonUtil.getParcelableExtra(intent, Const.EVENT_REPLY, EventReply::class.java)

        Glide.with(this).load(Const.API_URL+"profile/${mEventReply!!.userKey}").apply(RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE).centerCrop().placeholder(R.drawable.ic_profile_default).error(R.drawable.ic_profile_default)).into(binding.imageEventReplyEditProfileImage)

        binding.textEventReplyEditName.text = mEventReply!!.memberTotal!!.nickname
...
//        edit_event_reply_edit_contents.setSelection(mEventReply!!.reply!!.length)
    ...
    private fun updateEventReply() {
        showProgress("")
        ApiBuilder.create().updateEventReply(mEventReply!!.seqNo!!, mEventReply!!).setCallback(object : PplusCallback<NewResultResponse<Any>> {
