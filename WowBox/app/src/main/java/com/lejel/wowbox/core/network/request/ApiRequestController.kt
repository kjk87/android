...
import com.lejel.wowbox.core.util.PplusCommonUtil.Companion.hasNetworkConnection
...
class ApiRequestController private constructor() : INewApi {
    ...
    override fun <T> requestCall(tag: Any?,
        ...
        if (!hasNetworkConnection()) {
            ...
            builder.setTitle(WowBoxApplication.context.getString(R.string.word_notice_alert))
            builder.setStyle_alert(AlertBuilder.STYLE_ALERT.MESSAGE)
            builder.addContents(AlertData.MessageData(WowBoxApplication.context.getString(R.string.msg_disconnected_network), AlertBuilder.MESSAGE_TYPE.TEXT, 1))
            builder.addContents(AlertData.MessageData(WowBoxApplication.context.getString(R.string.msg_check_network_status), AlertBuilder.MESSAGE_TYPE.TEXT, 2))
            builder.setLeftText(WowBoxApplication.context.getString(R.string.word_confirm))
            builder.builder().show(WowBoxApplication.context, true)
            return
        }
    ...
    private inner class RequestCallback<T>(private val tag: Any?,
                        ...
                        if (resultResponseCallback != null) {
                            ...
                            if (resultResponseCallback is PplusforAlertCallback) {
                                ...
                                Handler(Looper.getMainLooper()).post {
                                    ToastUtil.showAlert(WowBoxApplication.context, WowBoxApplication.context.getString(R.string.server_error_default), true)
                                    try {
        ...
        fun showDefaultErrorAlert() {
            Handler(Looper.getMainLooper()).post {
                val value = WowBoxApplication.context.getString(R.string.server_error_default)
                ToastUtil.showAlert(WowBoxApplication.context, value, true)
                try {
                    DialogManager.instance.hideAll()
                } catch (e: Exception) {
                    LogUtil.e(LOG_TAG, e)
