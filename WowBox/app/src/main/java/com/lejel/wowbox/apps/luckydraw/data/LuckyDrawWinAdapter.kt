...
import com.lejel.wowbox.apps.luckydraw.ui.LuckyDrawWinReplyActivity
import com.lejel.wowbox.core.network.model.dto.LuckyDrawWin
import com.lejel.wowbox.core.util.PplusCommonUtil
import com.lejel.wowbox.databinding.ItemLuckyDrawWinBinding
...
class LuckyDrawWinAdapter() : RecyclerView.Adapter<LuckyDrawWinAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.binding.textLuckyDrawWinGiftPrice.text = holder.itemView.context.getString(R.string.format_money_unit, FormatUtil.getMoneyTypeFloat(item.giftPrice.toString()))


        holder.binding.textLuckyDrawWinNickname.text = item.memberTotal?.nickname
        Glide.with(holder.itemView.context).load(Const.API_URL+"profile/${item.userKey}").apply(RequestOptions().diskCacheStrategy(DiskCacheStrategy.NONE).centerCrop().placeholder(PplusCommonUtil.getDefaultProfile(position)).error(PplusCommonUtil.getDefaultProfile(position))).into(holder.binding.imageLuckyDrawWinProfile)
