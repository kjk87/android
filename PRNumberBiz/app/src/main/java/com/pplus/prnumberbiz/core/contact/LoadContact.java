    ...
    public boolean getPhoneNumbers(Context context, ContactDao contactDao){
        ...
        if(cursor != null && cursor.moveToFirst()) {
            do {
                ...
                if(LoginInfoManager.getInstance().isMember() && StringUtils.isNotEmpty(LoginInfoManager.getInstance().getUser().getMobile()) && strPhoneNumber.equals(LoginInfoManager.getInstance().getUser().getMobile())) {
                    continue;
                }
        ...
        if(mContactList.size() > 0) {
            ...
            if(contactList.size() == 0) {
                for(int i = 0; i < mContactList.size(); i++) {
                    ...
                    LogUtil.e("loadContact", "insert result : {}", contactDao.insertOrReplace(contact));
