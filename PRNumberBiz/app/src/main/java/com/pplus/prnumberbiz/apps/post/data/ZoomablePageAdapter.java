    ...
    public Object instantiateItem(ViewGroup container, final int position){
        ...
        if(StringUtils.isNotEmpty(item.getTargetType()) && item.getTargetType().equals("youtube")) {
            ...
            if(item.getWidth() != 0 && item.getHeight()!= 0) {
                ...
                Glide.with(mContext).load(glideUrl).apply(new RequestOptions().fitCenter().placeholder(R.drawable.prnumber_default_img).error(R.drawable.prnumber_default_img).override(DeviceUtil.DISPLAY.SCREEN_WIDTH_PIXELS, DeviceUtil.DISPLAY.SCREEN_HEIGHT_PIXELS))
                        .into(imageView);
            }
        }
